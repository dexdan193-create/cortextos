    ...
    try {
      while (this.running) {
        ...
        try {
          ...
          // The enclosing try/finally still tears down the watchdog on return.
          if (/Conflict/i.test(msg)) {
            this.lastExitReason = 'conflict-self-die';
            this.running = false;
            return;
          }
          // Other errors are transient comms failures — drive Guard #2 backoff
          // (component (i)) and, when comms-managed, escalation + in-process
          // restart, rather than just logging and continuing.
          failed = true;
